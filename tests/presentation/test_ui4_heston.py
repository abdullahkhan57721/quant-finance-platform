...
def test_heston_pricing_presentation_teaches_model_method_and_uncertainty_boundaries() -> None:
...
def test_real_market_reference_presentation_preserves_provenance_and_nonclaims() -> None:
    presentation = build_m6_market_reference_presentation(
        canonical_m6_market_reference()
    )

    assert any(row.label == "Pinned public source" for row in presentation.provenance_rows)
